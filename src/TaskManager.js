...
import { fetchTasksPost } from "./api.js";
...
export class TaskManager {
    ...
    // -> Añande las tareas que vienen del backend
    static addNewTaskBack(id, title, description, assigned, priority, limitDate, state) {
        const newTask = new Task(id, title, description, assigned, priority, limitDate, state);
        const newTaskState = newTask.state;
        let container;
        let list;
    ...
    static editTaskToEdit(title, description, assigned, priority, limitDate, state) {
        ...
        updateTaskApi(this.#TASK_TO_EDIT.id, {
            title: title,
            description: description,
            assignedTo: assigned,
            startDate: '1/1/1', 
    ...
    static searchTaskById(taskToSearchId) {
        taskToSearchId = String(taskToSearchId);  // Convertir el ID a cadena
        return this.#BACKLOG.find(task => String(task.id) === taskToSearchId) ||
               this.#TO_DO.find(task => String(task.id) === taskToSearchId) ||
               this.#IN_PROGRESS.find(task => String(task.id) === taskToSearchId) ||
               this.#BLOCKED.find(task => String(task.id) === taskToSearchId) ||
               this.#DONE.find(task => String(task.id) === taskToSearchId);
    }
